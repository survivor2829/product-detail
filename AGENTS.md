...
`.codex/` 目录是项目的 Codex 协作配置，已落地的内容如下：

### Agent 自主性约定（档 2 PR 模式）

详见 `.codex/` 下的协作配置（2026-05-06 由 master roadmap §6 定义）。
...
本机临时白名单不进 git。团队规则永远写进 `.codex/` 下的项目配置。

### 修改 .codex/ 后

```bash
# 验证 JSON 格式
python -c "import json; json.load(open('.codex/hooks.json', encoding='utf-8'))"
